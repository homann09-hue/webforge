    ...
    <main className={`${styles.demo} ${styles.gastro}`}>
      ...
      <section className={`${styles.hero} ${styles.shell}`} id="top">
        <div className={styles.heroCopy}>
          ...
          <p>
            Kunden wählen ihr Gericht, passen Größe und Extras an und bestellen direkt beim Restaurant. Kein Umweg über
            eine fremde Plattform nötig.
          </p>
          <div className={styles.heroActions}>
            <a className={styles.solidButton} href="#menu">Speisekarte öffnen ↗</a>
            <a className={styles.ghostButton} href="#kitchen">Bestellstatus ansehen</a>
          </div>
          <div className={styles.facts}>
            <div className={styles.fact}><b>25–40</b><span>Minuten Lieferzeit</span></div>
            <div className={styles.fact}><b>4,8 ★</b><span>Bewertung</span></div>
            <div className={styles.fact}><b>430°C</b><span>Pizzaofen</span></div>
          </div>
        </div>
        <div className={`${styles.stage} ${styles.gastroStage}`}>
          <div className={styles.stageTop}>
            <span>FORNO 37 / KÜCHE</span>
            <span className={styles.live}><i />Jetzt geöffnet</span>
          </div>
          <div className={styles.pizzaOrbit} />
          <div className={styles.stageCard}>
            <div><small>AKTUELLE BESTELLUNG</small><b>Burrata Club · L</b></div>
            <strong>13:42</strong>
          </div>
        </div>
      </section>
        ...
        <div className={styles.shell}>
          <div className={styles.sectionHead}>
            ...
            <div>
              ...
              <p>
                Größen und Extras lassen sich direkt beim Produkt auswählen. So bekommt der Kunde genau das, was er
                möchte, ohne anrufen zu müssen.
              </p>
            </div>
          </div>
              ...
              <article className={styles.menuCard} key={item.id}>
                ...
                <div className={styles.menuCopy}>
                  ...
                  <div className={styles.menuBuy}>
                    ...
                    <button type="button" aria-label={`${item.name} auswählen`} onClick={() => startConfig(item)}>+</button>
                  </div>
                </div>
              </article>
            ))}
        ...
        <div className={styles.shell}>
          <div className={styles.sectionHead}>
            ...
            <div>
              ...
              <p>
                Nach der Bestellung kann der aktuelle Stand angezeigt werden – vom Eingang bis zur Abholung oder
                Lieferung.
              </p>
            </div>
          </div>
          <div className={styles.kitchen}>
            {[
              [1, "Bestellung eingegangen", "Die Bestellung ist im Restaurant angekommen."],
          ...
          <div style={{ marginTop: 20 }}>
            <button
              ...
              type="button"
              onClick={() => {
                if (!cart.length) startConfig(menu[2]);
                else simulateOrder();
              }}
            >
        ...
        <div className={styles.shell}>
          <div className={styles.sectionHead}>
            ...
            <div>
              ...
              <p>
                Eine eigene Bestellseite kann dem Restaurant mehr Kontrolle geben und macht es Kunden trotzdem einfach,
                mobil zu bestellen.
              </p>
            </div>
          </div>
          <div className={styles.grid3}>
            {[
              ...
              ["Direkt beim Restaurant", "Die Bestellung läuft über die eigene Website statt über einen Marktplatz."],
              ["Fürs Handy gemacht", "Auswählen, anpassen und bestellen funktioniert bequem auf dem Smartphone."],
            ].map(([title, text], index) => (
              <article className={styles.card} key={title}>
                <div className={styles.cardNumber}><span>0{index + 1}</span><span>FORNO 37</span></div>
                <h3>{title}</h3>
                <p>{text}</p>
              </article>
            ))}
      ...
      <section className={styles.contact} id="info">
        <div className={`${styles.shell} ${styles.contactGrid}`}>
          <div>
            ...
            <p>
              Lieferung in Hildesheim und bis 8 km Umgebung. Mindestbestellwert 15 €. Ab 25 € ist die Lieferung in
              dieser Demo kostenlos.
            </p>
          </div>
          <form onSubmit={(event) => { event.preventDefault(); setDrawer(true); }}>
            <label htmlFor="g-mode">
              Bestellung
              <select id="g-mode" value={mode} onChange={(event) => setMode(event.target.value as "delivery" | "pickup")}>
      ...
      <footer className={`${styles.footer} ${styles.shell}`}>
        <div><strong>FORNO 37</strong><br />Pizza · Pasta · Hildesheim</div>
        ...
        <div className={styles.overlay} onClick={() => setConfig(null)}>
          <div className={styles.modal} onClick={(event) => event.stopPropagation()}>
            ...
            <div className={styles.optionRow} id="g-size">
              <button data-active={size === "M"} type="button" onClick={() => setSize("M")}>M · 28 cm</button>
              <button data-active={size === "L"} type="button" onClick={() => setSize("L")}>L · 32 cm +3 €</button>
            </div>
            ...
            <div className={styles.optionRow} id="g-extras">
              {Object.entries(extraPrices).map(([extra, price]) => (
                <button data-active={extras.includes(extra)} type="button" key={extra} onClick={() => toggleExtra(extra)}>
                  {extra} +{money(price)}
                </button>
              ))}
            </div>
            <div className={styles.result}>
              <small>PREIS</small>
              <b>{money(config.price + (size === "L" ? 3 : 0) + extras.reduce((total, extra) => total + extraPrices[extra], 0))}</b>
              <p>Alle ausgewählten Extras werden direkt übernommen.</p>
            </div>
            <button className={styles.solidButton} style={{ width: "100%", marginTop: 16 }} type="button" onClick={addConfigured}>
              In den Warenkorb ↗
            </button>
          </div>
        </div>
      ...
      {drawer && (
        <aside className={styles.drawer} aria-label="Warenkorb">
          <div className={styles.drawerHead}>
            <div><small>IHRE BESTELLUNG</small><h2>Warenkorb</h2></div>
            <button className={styles.drawerClose} type="button" onClick={() => setDrawer(false)}>×</button>
          </div>
          {!cart.length ? (
            <p style={{ color: "var(--muted)", marginTop: 30 }}>Der Warenkorb ist noch leer. Wählen Sie zuerst ein Gericht aus.</p>
          ) : (
            cart.map((item, index) => (
              <div className={styles.cartLine} key={`${item.id}-${index}`}>
                <div>
                  ...
                  <span>{money((item.price + (item.size === "L" ? 3 : 0) + item.extras.reduce((total, extra) => total + extraPrices[extra], 0)) * item.qty)}</span>
                </div>
          ...
          {cart.length > 0 && (
            <div className={styles.cartTotals}>
              <div><span>Zwischensumme</span><b>{money(subtotal)}</b></div>
              <div><span>{mode === "delivery" ? "Lieferung" : "Abholung"}</span><b>{money(delivery)}</b></div>
              <div className={styles.grand}><span>Gesamt</span><b>{money(subtotal + delivery)}</b></div>
              <button className={styles.solidButton} type="button" onClick={simulateOrder}>Bestellung testen ↗</button>
              <small>Nur eine Demo – keine Zahlung und keine echte Bestellung.</small>
            </div>
          )}
        </aside>
      )}
