...
export async function createLead(input: { company: string; email: string; website?: string }) {
  await rpc("submit_lead", { p_company: input.company, p_email: input.email, p_website: input.website || null });
}

export async function listLeads(password: string, limit = 100): Promise<Lead[]> {
  const response = await rpc("admin_list_leads", { p_password: password, p_limit: Math.min(Math.max(limit, 1), 200) });
  return (await response.json()) as Lead[];
}

export async function updateLeadStatus(password: string, leadId: number, status: LeadStatus) {
  await rpc("admin_update_lead_status", { p_password: password, p_lead_id: leadId, p_status: status });
}

export async function updateLeadNotes(password: string, leadId: number, notes: string) {
  await rpc("admin_update_lead_notes", { p_password: password, p_lead_id: leadId, p_notes: notes });
}

export async function markLeadContacted(password: string, leadId: number): Promise<string> {
  const response = await rpc("admin_mark_lead_contacted", { p_password: password, p_lead_id: leadId });
  return await response.json() as string;
}

export async function archiveLead(password: string, leadId: number, archived: boolean) {
  await rpc("admin_archive_lead", { p_password: password, p_lead_id: leadId, p_archived: archived });
}

export async function deleteLead(password: string, leadId: number) {
  await rpc("admin_delete_lead", { p_password: password, p_lead_id: leadId });
}
