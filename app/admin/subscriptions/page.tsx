...
export default function SubscriptionsAdmin() {
  ...
  const availableProjects = useMemo(
    () => projects.filter((project) => !leadId || project.lead_id === Number(leadId)),
    [projects, leadId],
  );
  ...
  async function loadAll() {
    ...
    try {
      const [subsData, leadsData, projectsData] = await Promise.all([
        post("/api/admin/subscriptions", { action: "list" }),
  ...
  async function generateDue() {
    ...
    try {
      ...
      setNotice(
        generated.length
          ? `${generated.length} Monatsrechnung(en) erzeugt: ${generated.map((item) => item.invoice_number).join(", ")}`
          : "Keine fälligen Monatsrechnungen.",
      );
      await loadAll();
    } catch (err) {
      ...
      <section>
        <div className="adminhead">
          <div>
            ...
            <h1>Monatliche Betreuung</h1>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button className="button" onClick={() => void generateDue()} disabled={loading}>
              Fällige Rechnungen erzeugen
            </button>
            <a className="button" href="/admin/invoices">
              Rechnungen
            </a>
          </div>
        </div>
        <div className="stats">
          ...
          <article>
            <small>ZAHLUNG OFFEN</small>
            <strong>{pastDue}</strong>
            <span>Stripe/Manuell</span>
          </article>
        </div>
        {error && (
          <div className="adminpanel">
        ...
        <div className="adminpanel">
          <small>NEUE BETREUUNG</small>
          <h2>Monatliches Abo anlegen</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 8 }}>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Leistung" />
            <input
              value={amount}
        ...
        <div className="adminpanel">
          <small>ABOS</small>
          <h2>{subscriptions.length} Verträge</h2>
          {subscriptions.length === 0 && <p>Noch keine monatlichen Betreuungen.</p>}
          {subscriptions.map((subscription) => (
            <div className="adminrow" key={subscription.id}>
              <span className="dot" />
              <div>
                <strong>
              ...
              <select
                ...
                onChange={(e) => void setStatus(subscription.id, e.target.value as BillingSubscriptionStatus)}
              >
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {subscription.stripe_subscription_id ? (
                  ...
                  <button
                    className="button"
                    disabled={busyId === subscription.id}
                    onClick={() => void createStripeCheckout(subscription.id)}
                  >
                    Stripe Checkout
                  </button>
                )}
                {subscription.stripe_checkout_url && (
                  <a className="button" href={subscription.stripe_checkout_url} target="_blank" rel="noreferrer">
                    Checkout öffnen
                  </a>
                )}
              </div>
