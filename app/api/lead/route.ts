...
export async function POST(req: Request) {
  try {
    ...
    if (company.length < 2 || company.length > 120 || !validEmail(email) || email.length > 254 || website.length > 300) {
      return NextResponse.json(
        { ok: false, error: "Bitte Unternehmen und gültige E-Mail angeben." },
        { status: 400 },
      );
    }
